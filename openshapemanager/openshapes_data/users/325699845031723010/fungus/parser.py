...
def create_character_config(shapes_data):
    ...
    personality_conversational_goals = shapes_data.get(
        "personality_conversational_goals", ""
    )
    personality_conversational_examples = shapes_data.get(
    ...
    initial_message = ""
    if (
        "shape_settings" in shapes_data
        and "shape_initial_message" in shapes_data["shape_settings"]
    ):
    ...
    if (
        "shape_settings" in shapes_data
        and "appearance" in shapes_data["shape_settings"]
    ):
    ...
    if personality_conversational_goals:
        character_scenario = personality_conversational_goals.replace(
            "{user}", "[user]"
        )
    ...
    config = {
        ...
        "personality_conversational_goals": personality_conversational_goals.replace(
            "{user}", "[user]"
        ),
        "personality_conversational_examples": personality_conversational_examples.replace(
            "{user}", "[user]"
        ),
...
def create_memory_json(brain_data, shapes_data):
    ...
    for entry in brain_data:
        if (
            entry["story_type"] == "general"
            and entry["content"] != "Insert general knowledge here"
        ):
            ...
            entry["story_type"] == "personal"
            and entry["content"]
            != 'Insert "personal" custom added sorting option for knowledge here'
        ):
            ...
            entry["story_type"] == "relationships"
            and entry["content"] != "Insert relationship here"
        ):
            ...
            entry["story_type"] == "commands"
            and entry["content"] != "Insert command here "
        ):
...
def save_json_files(parsed_data, output_dir="."):
    ...
    with open(
        os.path.join(output_dir, "character_config.json"), "w", encoding="utf-8"
    ) as f:
        json.dump(parsed_data["character_config"], f, indent=2)

    # Save memory.json
    with open(
        os.path.join(output_dir, "character_data", "memory.json"), "w", encoding="utf-8"
    ) as f:
...
def main():
    """
    Main function that automatically uses config.json as shapes_json_path
    and brain.json if it exists in the current directory.
    """
    shapes_json_path = "./config.json"
    brain_json_path = "./brain.json" if os.path.exists("./brain.json") else None
    output_dir = "."

    try:
        # Parse data
        print(f"Parsing shapes data from {shapes_json_path}")
        if brain_json_path:
            print(f"Using brain data from {brain_json_path}")
        else:
