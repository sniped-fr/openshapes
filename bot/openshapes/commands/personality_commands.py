...
class PersonalityCommandHandler:
    ...
    async def handle_personality_traits_edit(self, interaction: discord.Interaction) -> None:
        if not await PermissionChecker.check_owner_permission(interaction, self.bot.owner_id):
            return
    ...
    async def handle_preferences_edit(self, interaction: discord.Interaction) -> None:
        ...
        async def select_callback(select_interaction: discord.Interaction) -> None:
            ...
            preference_value = preference["values"][0]
            await self.editor.create_preference_modal(preference_value, select_interaction)
...
async def edit_personality_traits_command(self, interaction: discord.Interaction) -> None:
    handler = PersonalityCommandHandler(self)
    await handler.handle_personality_traits_edit(interaction)

async def edit_backstory_command(self, interaction: discord.Interaction) -> None:
    handler = PersonalityCommandHandler(self)
    await handler.handle_backstory_edit(interaction)

async def edit_preferences_command(self, interaction: discord.Interaction) -> None:
    handler = PersonalityCommandHandler(self)
